...
Example: If Lens A says "agent wasted time researching beat-link API" and
Lens C says "beat-link API documentation is missing" — that's one root cause
appearing in two lenses. The fix (create beat-link API reference doc) addresses
both efficiency and knowledge gaps.
  ...
  "ranked_improvements": [
    {
      ...
      "feasibility": 3,
      "score": 60,
      "suggested_type": "skill|hook|memory|doc|refactor|convention|eval-case"
    }
  ],
...
4. **Collect:** 12 lens reports total (3 lenses × 4 batches)
5. **Per-batch synthesis:** Run synthesis agent on each batch's 3 reports (4 runs)
...
**Cost estimate:** ~12 agent sessions for lens analysis + 5 for synthesis = 17 total.
At ~$2-3 per session, ~$35-50 for a full 20-session analysis cycle.
