...
> **Mirror copy.** The canonical version of this file lives at the workspace root:
> `PROTOCOL_IMPROVEMENTS.md`. This copy exists for `.agent/` completeness but
> should not be edited directly. Edit the root copy instead.
>
> **How to use:** When you notice something during a session — an agent forgot a step,
> a template is missing a field, a workflow has a gap — add a one-liner to the root copy.
